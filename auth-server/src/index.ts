...
// We wrap it to check admin status after OAuth callbacks
const authHandler = toNodeHandler(auth);
app.all("/api/auth/*", async (req, res, next) => {
  // Check if this is an OAuth callback
  const isOAuthCallback = req.path.includes('/callback/google') || req.path.includes('/callback/github');
  
  if (isOAuthCallback) {
    // Store original redirect
    const originalRedirect = res.redirect.bind(res);
    
    // Override redirect to check admin before redirecting
    res.redirect = function(url: string | number, url2?: string): express.Response {
      const finalUrl = typeof url === 'number' ? url2! : url as string;
      
      // Check admin status asynchronously before redirecting
      setTimeout(async () => {
        try {
          const sessionToken = req.cookies?.['better-auth.session_token'] || 
                               req.headers.cookie?.match(/better-auth\.session_token=([^;]+)/)?.[1];
      ...
      }, 300); // Small delay to ensure session is created
      
      // Proceed with original redirect
      return originalRedirect(finalUrl);
    };
  }
