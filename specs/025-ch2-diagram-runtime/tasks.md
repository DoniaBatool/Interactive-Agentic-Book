# Tasks: Chapter 2 — Diagram Generator Runtime

**Feature**: 025-ch2-diagram-runtime | **Branch**: `025-ch2-diagram-runtime` | **Date**: 2025-01-27
**Generated From**: [plan.md](./plan.md) | [spec.md](./spec.md)

**Purpose**: Convert implementation plan into atomic, executable tasks for creating Chapter 2 diagram generator runtime infrastructure scaffolding (runtime module, prompt template, routing, skills, API integration). All tasks are scaffolding only—no real AI logic implementation.
...
**Purpose**: Verify dependencies and prerequisites before creating Chapter 2 diagram runtime scaffolding.

- [ ] [T001] [P1] [SETUP] Verify Feature 008 (Chapter 1 Diagram Engine) exists: Check that `backend/app/ai/diagram/runtime.py` exists and is importable (for reference structure)
...
- [ ] [T003] [P1] [SETUP] Verify API layer exists: Check that `backend/app/api/ai_blocks.py` exists and has diagram endpoint
...
- [ ] [T006] [P1] [SETUP] Verify diagram directory exists: Check that `backend/app/ai/diagram/` directory exists (create if needed)
...
**Dependencies**: Feature 008 (Chapter 1 Diagram Engine) and Feature 024 (Chapter 2 Backend Runtime Wiring) must be complete

---

## PHASE 1 — Diagram Runtime Module

**User Story**: US1 - Developer Sets Up Chapter 2 Diagram Runtime Infrastructure
...
- [ ] [T007] [P1] [RUNTIME] Create `backend/app/ai/diagram/ch2_diagram_runtime.py`:
  - Add module docstring: `"""Chapter 2 Diagram Generator Runtime - Orchestrates diagram generation flow for ROS 2 diagrams"""`
  ...
  - Function docstring explaining:
    - Args: diagram_type, chapter_id (should be 2), concepts
...
- [ ] [T008] [P1] [RUNTIME] Verify ch2_diagram_runtime.py is importable: Run `cd backend && python -c "from app.ai.diagram.ch2_diagram_runtime import run; print('Import successful')"` - should complete without errors

- [ ] [T009] [P2] [RUNTIME] Verify ch2_diagram_runtime.py structure matches runtime.py (Chapter 1):
...
- [ ] [T010] [P1] [PROMPT] Create directory structure if needed: `backend/app/ai/prompts/diagram/` (create if doesn't exist)

- [ ] [T011] [P1] [PROMPT] Create `backend/app/ai/prompts/diagram/ch2_diagram_prompt.txt`:
  - Add header comment: `# TODO: Engineer full prompt template for Chapter 2 diagrams`
...
- [ ] [T012] [P1] [PROMPT] Verify ch2_diagram_prompt.txt is readable: Check that file exists and can be read by Python `open()` function
...
**User Story**: US2 - System Routes Chapter 2 Diagram Requests

**Test Strategy**: Can be tested by verifying runtime engine has Chapter 2 diagram routing comments and TODO markers.

### Runtime Engine Updates

- [ ] [T013] [P1] [ROUTING] Update `backend/app/ai/runtime/engine.py` with Chapter 2 diagram routing:
  - Locate `run_ai_block()` function
  - Add routing case (comments only, before existing diagram routing if present):
    ```python
    ...
    #     return result
    ```
  - Ensure routing condition checks both `block_type == "diagram"` AND `chapterId == 2`
  - Add comment explaining: `# Routes Chapter 2 diagram requests to ch2_diagram_runtime (placeholder routing)`
...
**Acceptance Test**: Runtime engine has Chapter 2 diagram routing (comments only), routing condition checks both block_type and chapterId, file still imports correctly, TODO comments are present
...
- [ ] [T016] [P1] [API] Update `backend/app/api/ai_blocks.py` diagram endpoint with Chapter 2 routing:
  - Locate `diagram()` function (POST /api/ai/diagram endpoint)
  - Add routing comment (before existing routing logic):
    ...
    #     return result
    ```
  - Add TODO documentation tag: `# TODO: Route Chapter 2 diagram requests to ch2_diagram_runtime`
  - Ensure routing supports chapterId=2 (may already be generic)

- [ ] [T017] [P1] [API] Verify ai_blocks.py still imports correctly: Run `cd backend && python -c "from app.api.ai_blocks import diagram; print('Import successful')"` - should complete without errors

- [ ] [T018] [P2] [API] Verify endpoint accepts chapterId=2: Check that DiagramRequest model supports chapterId parameter (should already exist)
...
- [ ] [T019] [P1] [DOCS] Verify `specs/025-ch2-diagram-runtime/contracts/diagram-contract.yaml` exists:
  - Check that file was created during spec phase
  - Verify file contains:
    - Chapter 2 diagram types documentation
...
**Acceptance Test**: Contract file exists, documents expected placeholders, no schemas for actual diagram formats
...
- [ ] [T021] [P1] [SKILLS] Update `backend/app/ai/skills/prompt_builder_skill.py` with build_diagram_prompt_ch2() function:
  - Add function definition: `def build_diagram_prompt_ch2(diagram_type: str, chapter_id: int, concepts: List[str]) -> str:`
  ...
  - Add TODO comment: `# TODO: Implement prompt building for Chapter 2 diagrams`

- [ ] [T022] [P1] [SKILLS] Verify prompt_builder_skill.py still imports correctly: Run `cd backend && python -c "from app.ai.skills.prompt_builder_skill import build_diagram_prompt_ch2; print('Import successful')"` - should complete without errors

### Formatting Skill

- [ ] [T023] [P1] [SKILLS] Update `backend/app/ai/skills/formatting_skill.py` with format_diagram_output_ch2() function:
  - Add function definition: `def format_diagram_output_ch2(raw_response: Dict[str, Any]) -> Dict[str, Any]:`
  - Add function docstring:
    ```python
    """
  ...
  - Add TODO comment: `# TODO: Implement formatting for Chapter 2 diagram output`

- [ ] [T024] [P1] [SKILLS] Verify formatting_skill.py still imports correctly: Run `cd backend && python -c "from app.ai.skills.formatting_skill import format_diagram_output_ch2; print('Import successful')"` - should complete without errors
...
- [ ] [T026] [P1] [VALIDATE] Test all new module imports:
  - Test ch2_diagram_runtime: `python -c "from app.ai.diagram.ch2_diagram_runtime import run; print('ch2_diagram_runtime: OK')"`
  - Test prompt template: `python -c "with open('backend/app/ai/prompts/diagram/ch2_diagram_prompt.txt') as f: print('ch2_diagram_prompt.txt: OK')"`
  - Test skills functions: `python -c "from app.ai.skills.prompt_builder_skill import build_diagram_prompt_ch2; from app.ai.skills.formatting_skill import format_diagram_output_ch2; print('Skills: OK')"`
  - All imports should succeed

### Logic Verification

- [ ] [T027] [P1] [VALIDATE] Verify no real AI logic implemented:
  - Check ch2_diagram_runtime.py: All steps should have TODO markers only
  - Check prompt_builder_skill.py: build_diagram_prompt_ch2() should return placeholder
  - Check formatting_skill.py: format_diagram_output_ch2() should return placeholder
  - Check engine.py: Chapter 2 routing should be comments only
  - Check ai_blocks.py: Chapter 2 routing should be comments only

- [ ] [T028] [P1] [VALIDATE] Verify Chapter 1 diagram functionality still works:
...
- [ ] [T029] [P1] [VALIDATE] Verify all required files exist:
  - [ ] `backend/app/ai/diagram/ch2_diagram_runtime.py` exists
  - [ ] `backend/app/ai/prompts/diagram/ch2_diagram_prompt.txt` exists
  - [ ] `specs/025-ch2-diagram-runtime/contracts/diagram-contract.yaml` exists
